...
{
public:
	virtual void unify(long long id1, long long id2) = 0;
	virtual long long find(long long id) = 0;
	virtual long long get_components_count() = 0;
	virtual map<long long, vector<long long>> get_components() = 0;
};

class dsu :public dsu_interface
{
private:
	long long compnents_count;
	vector<long long>id_of_compnent;
	vector<long long>size_of_compnent;
public:
	dsu(long long elements_count)
	{
		id_of_compnent.resize(elements_count);
		size_of_compnent.resize(elements_count);
		this->compnents_count = elements_count;

		for (long long i = 0; i < elements_count; i++)
		{
			id_of_compnent[i] = i;
			size_of_compnent[i] = 1;
		}
	}
	long long find(long long element_id)override
	{
		auto root = element_id;
		...
		{
			auto next = id_of_compnent[element_id];
			id_of_compnent[element_id] = root;
			element_id = next;
		}
		return element_id;
	}
	void unify(long long id1, long long id2)override
	{
		long long root1 = this->find(id1);
		long long root2 = this->find(id2);
	...
	map<long long, vector<long long>> get_components() override
	{
		set<long long> st;
		for (long long i : this->id_of_compnent)
			st.insert(i);
		map<long long, long long> ordinal_id_of;
		long long id = 0;
		for (long long i : st)
			ordinal_id_of[i] = id++;

		map<long long, vector<long long>>components;
		for (long long i = 0; i < this->id_of_compnent.size(); i++)
			components[ordinal_id_of[this->id_of_compnent[i]]].push_back(i);
		return components;
	}
	long long get_components_count() override
	{
